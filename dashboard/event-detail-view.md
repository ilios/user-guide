...
* **Histology of the Cardiovascular System** - ILM due prior to "Response of the Heart to Injury". The logged-in user will be taken to the Event Detail view of the ILM when clicked.
...
There are lists of objectives available here on the Event Detail screen which can be expanded (default for Session objective) or contracted (default for Course objectives). Both of these can be listed as grouped by Competencies (the initial setting) or by Priority. The ordering of these sorted by priority is set in Courses and Sessions. Competency listings are sorted ascending (A to Z) with the associated objectives listed grouped inside the Competencies. The sort order for [course objectives](https://iliosproject.gitbook.io/ilios-user-guide/courses-and-sessions/courses/course_objectives/sort-objectives) as well as that of [session objectives](https://iliosproject.gitbook.io/ilios-user-guide/courses-and-sessions/sessions/session_objectives/sort-objectives) can be customized as described in the links provided.
